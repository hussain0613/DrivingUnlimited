...
#define RD_Q 5
	...
	int road_timer, min_speed, max_speed, ac, rd_x1, rd_x2; // level main theke othoba bahir theke initialize korte hbe
	int level, initial_level, dLevel;
	int score, life, last_hit, active_npc_q;
	int player_car_index, player_speed, ps_x, ps_y;
	int sc_timer;
	...
	void spawn_road(int i, int tier){
		...
		if(tier == -1){
			...
			if(i == 0) roads[i].y = (roads[RD_Q-1].y + SCRN_HEIGHT);
			else roads[i].y = roads[i-1].y + SCRN_HEIGHT;	
		}
	...
	void controll_player_speed()
	{
		...
		if(arrow_states[GLUT_KEY_RIGHT-100]) {
			if(theta > -5 && player_speed >0) theta -= 1;
			else if(theta < 5 && player_speed<0) theta += 1;
		}
		else{
			if(theta < 0) theta += .5;
		}
		...
		// speed er upangsho nirnoy
		ps_y = player_speed * cos(theta*(3.14/180));
		ps_x = -player_speed * sin(theta*(3.14/180));
	}

	void player_movement(){

		if((player.car.y >= 150) && (player.car.y+player.car.height) < (SCRN_HEIGHT-250))
			player.car.y += ps_y;
		if((player.car.x - (player.car.width/2) >= rd_x1) && (player.car.x+player.car.width*1.5 < rd_x2))
			player.car.x += ps_x;
		

		if(player_speed == 0 && player.car.y < player.y){
			++player.car.y;
		}
	...
	void npc_movement()
	{
		...
		for(int i=0; i < active_npc_q; ++i){
			...
			npc_right[i].car.y -= (npc_right[i].speed + ps_y );
		}
	...
	void keyboard(int key)
	{
		...
		if(key == ' ') space_down = true;
	}

	void keyboardUp(int key){
		if(key == ' ') space_down = false;
	}
