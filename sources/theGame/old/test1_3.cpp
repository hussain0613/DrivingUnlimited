...
#define CARS_Q 7
	...
	void spawn_road(int i, int tier){
		...
		else if(0<=tier && tier<3)
			roads[i].y = SCRN_HEIGHT*(tier-1);
		else if (tier >=3){
			if(i == 0) roads[i].y = (roads[2].y + SCRN_HEIGHT);
			else roads[i].y = roads[i-1].y+SCRN_HEIGHT;
	...
	void road_movement()
	{
		for(int i = 0; i<3; ++i){
			...
			else if(roads[i].y > SCRN_HEIGHT) spawn_road(i, -1);
	...
	void player_movement(){
		...
		if(arrow_states[GLUT_KEY_UP-100]){
			...
			//if (player_speed+2 <= max_speed) player_speed += 1;
		...
		if(arrow_states[GLUT_KEY_RIGHT-100] && side) {
			
			if(player.car.x+player.car.width*1.5 < rd_x2)
				player.car.x += 5;
		}
		if(arrow_states[GLUT_KEY_LEFT-100] && side) {
		...
		if(!var && player.car.y > player.y) --player.car.y;
		side = false;
		var = false;
	}
	...
	void scorer()
	{
		//if(game_on && !paused && player.car.y != player.y){
		if(game_on && !paused){
			score += (double)level*((double)player_speed/(double)max_speed);
	...
	void npc_spawn_left(int i)
	{
		...
		for(int j = 0; j<MX_NPC_Q; ++j){
			if(j != i){
				...
				y2 = npc_left[j].car.y;
				//if(j>i) y2 += (npc_left[j].speed -player_speed - 1); // j>i hoar maane car1 spawn howar por car2 er actual position change hbe.. tai eita add kora hoise
				
				w2 = npc_left[j].car.width;
				h2 = npc_left[j].car.height;
				speed2 = npc_left[j].speed;

				//flag = collide(x-10, y-10, w+20, h+20+dy, x2, y2, w2, h2);
				flag = collide(x-10, y-10, w+20, h+20, x2, y2, w2, h2+dy);
	...
	void npc_spawn_right(int i)
	{
		...
		for(int j = 0; j<MX_NPC_Q; ++j){
			if(j != i){
				...
				//if(j>i) y2 -= (npc_right[j].speed + max_speed + 1 + ac);
				
				w2 = npc_right[j].car.width;
				h2 = npc_right[j].car.height;
				speed2 = npc_right[j].speed;

				//flag = collide(x-10, y-10-dy, w+20, h+20+2*dy, x2, y2, w2, h2);
				flag = collide(x-10, y-10, w+20, h+20, x2, y2-dy, w2, h2+dy);
				while(flag){ //flag = 0 hoile ok.. otherwise kaaj korte hbe
					if(flag == 1 ){ // 1 hoile full collision
	...
	void setup()
	{
		...
		player_speed = 0;//(min_speed+max_speed)/2 + level;//jokhon level baarbe tokhon eta abar barate hbe
	...
	void draw()
	{
		...
		if(game_on && !paused){
			...
			if(!(frame%20)){
				...
				if(player_speed<0) player_speed+=2;
			}
		}
		++frame;
	}
