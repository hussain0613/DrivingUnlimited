...
#define PX_TO_KM (6.0/100000.0)
	...
	int min_speed, max_speed, player_speed, rd_x1, rd_x2; // level main theke othoba bahir theke initialize korte hbe
	...
	ige::Image rd_asphalt, rd_lefts[3], rd_rights[3], rd_mids[3], rd_quarter, cars[CARS_Q], cars_f[CARS_Q];
	
	Road roads[RD_Q];
	Car player, npc_left[MX_NPC_Q], npc_right[MX_NPC_Q];

	double theta, dur, dist; //test_drive

	bool arrow_states[4], space_down;
	...
	void load_images()
	{
		...
		rd_lefts[2].set_all(0, 0, SCRN_WIDTH, SCRN_HEIGHT, "images/ROAD/ROAD.png");
		rd_lefts[2].setup();
		...
		rd_rights[2].set_all(0, 0, SCRN_WIDTH, SCRN_HEIGHT, "images/ROAD/ROAD-2.png");
		rd_rights[2].setup();
		...
		// cars
		cars[0].set_all(0, 0, 36, 80, "images/cars/car_1.png");
		...
		cars[7].set_all(0, 0, 36, 80, "images/cars/CAR.png");
		
		cars[8].set_all(0, 0, 37, 80, "images/cars/CAR1.png");
		
		cars[9].set_all(0, 0, 39, 85, "images/cars/CAR3.png");
		
		cars[10].set_all(0, 0, 37, 75, "images/cars/CAR4.png");
		
		cars[11].set_all(0, 0, 44, 93, "images/cars/CAR5.png");
		
		cars[12].set_all(0, 0, 44, 90, "images/cars/CAR6.png");
		...
		cars_f[7].set_all(0, 0, 36, 80, "images/cars/CAR_f.png");
		
		cars_f[8].set_all(0, 0, 37, 80, "images/cars/CAR1_f.png");
		
		cars_f[9].set_all(0, 0, 39, 85, "images/cars/CAR3_f.png");
		
		cars_f[10].set_all(0, 0, 37, 75, "images/cars/CAR4_f.png");
		
		cars_f[11].set_all(0, 0, 44, 93, "images/cars/CAR5_f.png");
		
		cars_f[12].set_all(0, 0, 44, 90, "images/cars/CAR6_f.png");
	...
	void controll_player_speed()
	{
		if(arrow_states[GLUT_KEY_UP-100]){
			if (player_speed < max_speed) player_speed += 1;
		}
		if(arrow_states[GLUT_KEY_DOWN-100]){
			if(player_speed > -min_speed) player_speed -= 1;
		}
	}
	...
	void player_movement(){
		...
		if(space_down){
			if(player_speed<0) ++player_speed;
			if(player_speed>0) --player_speed;
		}

		dist += abs(player_speed);
	}
	...
	void scorer()
	{
		if(game_on && !paused){
			...
			if((dLevel+1)*(dLevel * .25 +1 ) * 500 <= score) {
				...
				if(life<MX_LIFE && dLevel%3 == 0) ++life;
	...
	void npc_spawn_left(int i)
	{
		...
		speed = rand()%((max_speed-3) - min_speed)+min_speed; // player er max_speed theke jeno kom thake
		...
		for(int j = 0; j<active_npc_q; ++j){
			if(j != i){
				...
				flag = collide(x-5, y-5, w+10, h+10, x2, y2, w2, h2);
	...
	void npc_spawn_right(int i)
	{
		...
		speed = rand()%((max_speed-3) - min_speed)+min_speed; //base speed ,+2 to make sure speed is more than road*/
	...
	void npc_movement()
	{
		...
		for(int i=0; i < active_npc_q; ++i){
			dy = (npc_left[i].speed - player_speed);
		...
		for(int i=0; i < active_npc_q; ++i){
			...
			npc_right[i].car.y -= (npc_right[i].speed + player_speed);
		}
	...
	void keyboard(int key)
	{
		...
		if(key == ' ')
			space_down = true;
		if(key == 'a') arrow_states[LEFT] = true;
		if(key == 'w') arrow_states[UP] = true;
		if(key == 'd') arrow_states[RIGHT] = true;
		if(key == 's') arrow_states[DOWN] = true;
	}
	...
	void keyboardUp(int key){
		...
		if(key == 'a') arrow_states[LEFT] = false;
		if(key == 'w') arrow_states[UP] = false;
		if(key == 'd') arrow_states[RIGHT] = false;
		if(key == 's') arrow_states[DOWN] = false;
	}
	...
	void setup()
	{
		...
		/*last_hit = 0;
		min_speed = 5; level = 1; max_speed = 10;
		initial_level = 1;
	...
	void start()
	{
		...
		min_speed = 3+level; max_speed = 13 + level*2;
	...
	void game_over()
	{
		...
		row.score = score;
		row.dist = dist; //saving in pixels
		row.dur = dur;
		row.start_level = initial_level;
		row.finish_level = level;
		row.car = player_car_index;
	}
	...
	void draw()
	{
		...
		//speed
		iSetColor(50, 50, 50);
		iFilledRectangle(rd_x2+15, 40, 225, 35);
		iSetColor(200, 200, 200);
		iText(rd_x2+25, 50, "Speed: ", GLUT_BITMAP_TIMES_ROMAN_24);
		iText(rd_x2+95,  50, (char*)std::to_string((long double) (player_speed*PX_TO_KM)*50*3600).c_str(), GLUT_BITMAP_TIMES_ROMAN_24);
		iText(rd_x2+95+80, 50, "km/h", GLUT_BITMAP_TIMES_ROMAN_24);

		//distance
		iSetColor(50, 50, 50);
		iFilledRectangle(10, 40, 250, 35);
		iSetColor(200, 200, 200);
		iText(20, 50, "Distance: ", GLUT_BITMAP_TIMES_ROMAN_24);
		iText(20+100, 50, (char*)std::to_string((long double) (dist*PX_TO_KM)).c_str(), GLUT_BITMAP_TIMES_ROMAN_24);
		iText(120+100, 50, "km", GLUT_BITMAP_TIMES_ROMAN_24);
